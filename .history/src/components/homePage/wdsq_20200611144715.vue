    ...
    <div class="tableWrap">
      <table style="width: 590px">
        ...
        <tr v-for="item in items" :key="item.busRemindId">
          <td>{{ item.busName }}</td>
          <td class="{{ item.busStatus==0?'未审核' : (item.busStatus==1?'审核通过' : (item.busStatus==2?'审核未通过' : '重新提交')) }}">{{ item.busStatus==0?'未审核' : (item.busStatus==1?'审核通过' : (item.busStatus==2?'审核未通过' : '重新提交')) }}</td>
