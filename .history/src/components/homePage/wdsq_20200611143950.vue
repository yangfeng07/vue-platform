    ...
    <div class="tableWrap">
      <table style="width: 590px">
        ...
        <tr v-for="item in items" :key="item.busRemindId">
          <td>{{ item.busName }}</td>
          <td>{{ item.busStatus==0?未审核 : (item.busStatus==1?<span style="color: green">审核通过</span> : (item.busStatus==2?<span style="color: red">审核未通过</span> : <span style="color: yellow">重新提交</span>)) }}</td>
